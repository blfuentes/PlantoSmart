...
#define TELEGRAM_REPORT_PERIOD_MS 30000
...
static char g_telegram_chat_id[TELEGRAM_CHAT_ID_MAX_LEN];
...
static void telegram_report_task(void* arg) {
    ...
    for (;;) {
        ...
        ESP_LOGI("TELEGRAM_TASK", "[%d] Sending: light=%d%% humidity=%d%%",
                 ++send_count,
                 (int)msg.data.light_percentage,
                 (int)msg.data.humidity_level);
        esp_err_t ret = telegram_bot_send_message(&g_telegram_client, g_telegram_chat_id, text);
        if (ret != ESP_OK) {
            ESP_LOGE("TELEGRAM_TASK", "Send failed: %s", esp_err_to_name(ret));
        } else {
            strncpy(last_sent_text, text, sizeof(last_sent_text) - 1);
            last_sent_text[sizeof(last_sent_text) - 1] = '\0';
            has_last_sent_text = true;
            ESP_LOGI("TELEGRAM_TASK", "Send #%d OK", send_count);
        }
    }
}
...
static void display_task(void* arg) {
    ...
    for (;;) {
        ...
        if (ssid) {
            snprintf(g_sysdevs->display.lines[DISPLAY_DEBUG_PAGE], DISPLAY_BUFFER_SIZE,
                     "WiFi: %s", ssid);
        } else {
            snprintf(g_sysdevs->display.lines[DISPLAY_DEBUG_PAGE], DISPLAY_BUFFER_SIZE,
                     "LDR:%4d", (int)msg.data.light_level);
        }
...
void app_main(void) {
    ...
    if (config_load_telegram_bot_token(telegram_token, sizeof(telegram_token))) {
        ...
        if (telegram_bot_client_init(&g_telegram_client, telegram_token) == ESP_OK) {
            ESP_LOGI("MAIN", "Loading Telegram chat ID...");
            g_telegram_ready = config_load_telegram_chat_id(g_telegram_chat_id,
                                                             sizeof(g_telegram_chat_id));
            if (g_telegram_ready) {
                ESP_LOGI("MAIN", "Telegram bot client initialized, chat_id=%s", g_telegram_chat_id);
            } else {
                ESP_LOGW("MAIN", "Telegram token OK but chat_id missing — set TELEGRAM_CHAT_ID in .env");
            }
        } else {
